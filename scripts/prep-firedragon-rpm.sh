...
cp -pf "${git_source_dir}"/cachyos-source/gentoo/0016-bmo-1516081-Disable-watchdog-during-PGO-builds.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/remove_addons.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/unity_kde/mozilla-kde.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/unity_kde/firefox-kde.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/unity_kde/unity-menubar.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/sed-patches/disable-pocket.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/sed-patches/allow-searchengines-non-esr.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/sed-patches/stop-undesired-requests.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/urlbarprovider-interventions.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/allow-ubo-private-mode.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/custom-ubo-assets-bootstrap-location.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/pref-naming.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/remap-links.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/hide-default-browser.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/lw-logo-devtools.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/privacy-preferences.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/remove-branding-urlbar.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/remove-cfrprefs.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/remove-organization-policy-banner.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/remove-snippets-from-home.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/website-appearance-ui-rfp.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/ui-patches/handlers.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/disable-data-reporting-at-compile-time.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/hide-passwordmgr.patch "${src_rpm_dir}"
cp -pf "${git_source_dir}"/source-librewolf/faster-package-multi-locale.patch "${src_rpm_dir}"
